    ...
    <div data-testid={`card-user-${user.id}`}>
      {isSelectable && (
        <input
          type="checkbox"
          checked={isSelected}
          onChange={(e) => onToggleSelect?.(user, e.target.checked)}
...
const mockSearchResult = {
  ...
  items: [
    createMockUser(1, 'testuser1'),
    createMockUser(2, 'testuser2')
  ]
}
...
describe('App', () => {
  ...
  it('shows no results message when search returns empty results', async () => {
    const user = userEvent.setup()
    vi.mocked(GithubApi.searchProfiles).mockResolvedValue({ total_count: 0, incomplete_results: false, items: [] })
  ...
  describe('Edit Mode', () => {
    ...
    it('allows selecting all users with master checkbox', async () => {
      ...
      const masterCheckbox = checkboxes.find(checkbox =>
        checkbox !== editModeCheckbox &&
        !checkbox.getAttribute('data-testid')?.includes('checkbox-')
      )
      ...
      const cardCheckboxes = screen.getAllByRole('checkbox').filter(cb =>
        cb.getAttribute('data-testid')?.includes('checkbox-')
      )
  ...
  it('retries API call when retry button is clicked', async () => {
    const user = userEvent.setup()
    const mockResponse = new Response('', { status: 403 })
    vi.mocked(GithubApi.searchProfiles)
      .mockRejectedValueOnce(mockResponse)
